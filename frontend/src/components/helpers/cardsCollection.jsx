...
class CardsCollection extends Component {
	...
	drawRandomCard() {
		this.getUnusedCards();
		var len = this.state.unusedCards.length
		if (len == 0) {
		...
		var card = this.state.unusedCards[randIndex];
		const data = {
	...
	renderCards () {
        if (!Array.isArray(this.state.userCards)) {
            return (
				<div>
					Der User hat noch keine Karten gezogen.
				</div>
			);
		}
		return this.state.userCards.map(card => {
            return (
				<div key={card.id}>
					<CardsRow header={card.contentjson.Header} text={card.contentjson.Text} flag={card.contentjson.Flag} useFlag={card.useflag}/>
				</div>
            );
		});
	}
	...
	render() {
		var currentUser = "Current User: " + this.state.currentUser
		return (
			<Grid
			container
			direction="column"
            ...
            currentUser: this.state.tmpUser
		}); this.getUserCards(); this.forceUpdate()}} style={{marginLeft: '20px'}}>
						Reload Cards
					</Button>
				</Grid>
				<Grid item>
				...
				<Grid item>
					<Button variant="contained" color='secondary' onClick={() => {if (window.confirm("Reset game?")){this.resetGame()}}}>
						Reset Game
					</Button>
					<Button variant="contained" onClick={() => window.location.reload(true)} style={{marginLeft: '20px'}}>
						Reload
					</Button>
				</Grid>
			</Grid>
